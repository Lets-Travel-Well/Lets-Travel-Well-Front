...
document.getElementById("search-area").addEventListener("change", function () {
    ...
    if (areaCode != 0) {
        ...
        "https://apis.data.go.kr/B551011/KorService1/areaCode1?serviceKey=" +
        serviceKey +
        "&numOfRows=20&pageNo=1&MobileOS=ETC&MobileApp=AppTest&areaCode=" +
        areaCode +
        "&_type=json";
        fetch(sigunAreaUrl, { method: 'GET' })
            .then((response) => response.json())
            .then((data) => makeOptionForSigun(data));
    } else {
...
document.getElementById("btn-search").addEventListener("click", () => {
    ...
    let areaCode = document.getElementById("search-area").value;
    let sigunCode = document.getElementById("search-sigun").value;
    ...
    if (parseInt(sigunCode)) {
        searchUrl += `&sigunguCode=${sigunCode}`;
    } else {
        alert("시군을 선택해주세요.");
    }
...
function makeList(data) {
    ...
    let trips = data.response.body.items.item; // trips => 여행지 정보들 출력 
    let tripList = ``;
    positions = [];
    trips.forEach((area) => {
        tripList += `
